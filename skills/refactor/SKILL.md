...
0. **Clarify** - A refactor's target shape is rarely fixed by the request. Before exploring, run one clarifying round through the host's question tool: target shape, what must not change, and what "done" looks like; recommendation first on each
