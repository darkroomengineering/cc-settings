...
case "$2:$3" in
  ...
  remove:*) printf '${concurrentSettings}' > "$HOME/.claude/settings.json"; printf '${concurrentGlobal}' > "$HOME/.claude.json"; rm -f "$HOME/.claude/.cc-settings-version"; mkdir "$HOME/.claude/.cc-settings-version"; touch "$HOME/.codex-edited-claude-settings"; cp "$plugin" "$plugin.stash" 2>/dev/null; rm -f "$plugin"; exit 0;;
  add:*) if [ -e "$plugin.stash" ]; then cat "$plugin.stash" > "$plugin"; else realpath "$source" > "$plugin"; fi; exit 0;;
esac
exit 0
`,
      );
...
case "$2:$3" in
  list:*) [ -e "$plugin" ] && printf '{"installed":[{"pluginId":"darkroom@cc-settings","installed":true,"enabled":true,"source":{"source":"local","path":"%s"},"marketplaceSource":{"sourceType":"local","source":"%s"}}]}\n' "$(esc "$(cat "$plugin")")" "$(esc "$(cat "$plugin")")" || printf '{"installed":[]}\n'; exit 0;;
  marketplace:list) [ -e "$market" ] && printf '{"marketplaces":[{"name":"cc-settings","root":"%s","marketplaceSource":{"sourceType":"local","source":"%s"}}]}\n' "$(esc "$(cat "$market")")" "$(esc "$(cat "$market")")" || printf '{"marketplaces":[]}\n'; exit 0;;
  marketplace:add) if [ -e "$market.stash" ]; then cat "$market.stash" > "$market"; else realpath "$source" > "$market"; fi; exit 0;;
  marketplace:remove) cp "$market" "$market.stash" 2>/dev/null; rm -f "$market"; exit 0;;
  add:*) if [ "${failingAction}" = add ] && [ ! -e "$HOME/.codex-early-failure-fired" ]; then printf '${concurrentSettings}' > "$HOME/.claude/settings.json"; printf '${concurrentGlobal}' > "$HOME/.claude.json"; realpath "$source" > "$plugin"; touch "$HOME/.codex-early-failure-fired"; echo plugin add failure >&2; exit 1; fi; if [ -e "$plugin.stash" ]; then cat "$plugin.stash" > "$plugin"; else realpath "$source" > "$plugin"; fi; exit 0;;
  remove:*) cp "$plugin" "$plugin.stash" 2>/dev/null; if [ "${failingAction}" = remove ] && [ ! -e "$HOME/.codex-early-failure-fired" ]; then printf '${concurrentSettings}' > "$HOME/.claude/settings.json"; printf '${concurrentGlobal}' > "$HOME/.claude.json"; rm -f "$plugin"; touch "$HOME/.codex-early-failure-fired"; echo plugin remove failure >&2; exit 1; fi; rm -f "$plugin"; exit 0;;
esac
exit 0
`,
        );
