...
| `pre-commit-sonor.ts` | Runs `sonor ratchet --changed` before `git commit`, if the target repo has [sonor](https://github.com/darkroomengineering/sonor) as a dependency and installed (`node_modules/.bin/sonor`). Blocks the commit on a red gate (`--dry-run`/`--help` exempt). Skips silently if sonor isn't a dependency, isn't installed, or its own git hooks are already active for the repo (`core.hooksPath`/`.git/hooks/pre-commit` carrying the `sonor-managed` marker) — this hook complements, never replaces, sonor's own repo-level hooks and CI check; it just makes Claude Code a good citizen when a repo declared sonor but never ran `sonor install-hooks`. The whole point of sonor is enforcement that works *without* Claude Code in the loop — this hook only makes CC catch it earlier, one commit sooner than the hook sonor itself would have installed | No |
