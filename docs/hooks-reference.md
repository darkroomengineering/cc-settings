...
| `pre-commit-sondeo.ts` | Runs `sondeo ratchet --changed` before `git commit`, if the target repo has [sondeo](https://github.com/darkroomengineering/sondeo) as a dependency and installed (`node_modules/.bin/sondeo`). Blocks the commit on a red gate (`--dry-run`/`--help` exempt). Skips silently if sondeo isn't a dependency, isn't installed, or its own git hooks are already active for the repo (`core.hooksPath`/`.git/hooks/pre-commit` carrying the `sondeo-managed` marker) — this hook complements, never replaces, sondeo's own repo-level hooks and CI check; it just makes Claude Code a good citizen when a repo declared sondeo but never ran `sondeo install-hooks`. The whole point of sondeo is enforcement that works *without* Claude Code in the loop — this hook only makes CC catch it earlier, one commit sooner than the hook sondeo itself would have installed | No |
