...
**Fan-out limits (upstream v2.1.217)**: at most 20 subagents run concurrently by default (`CLAUDE_CODE_MAX_CONCURRENT_SUBAGENTS` overrides; excess spawns queue), and subagents no longer spawn nested subagents unless `CLAUDE_CODE_MAX_SUBAGENT_SPAWN_DEPTH` allows it. cc-settings ships depth `2` in `config/10-core.json` — without it, `maestro` and `deslopper` (agents that themselves fan out via the Agent tool) would silently lose their orchestration ability when invoked as subagents.
