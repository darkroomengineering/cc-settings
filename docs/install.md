...
Every installer flag is forwarded (`npx darkroom-settings --target=claude --light`); `bunx darkroom-settings`
...
| Claude plugin store (`fast-jev-compaction@fast-jev-compaction`, `compaction-trigger@cc-settings`) | Sets `CLAUDE_CODE_ENABLE_FUNCTION_HOOKS=1` in `settings.json` env, registers the `fast-jev-compaction` and `cc-settings` marketplaces, and installs both plugins via the `claude` CLI when it's on PATH. A TypeSafe key unlocks verbatim Jev compaction: an interactive install prompts for it once (input hidden; Enter skips), `--typesafe-key=<key>` supplies it non-interactively, and either way it is stored through the plugin's sensitive `apiKey` option and written as `TYPESAFE_API_KEY` into the settings `env` block (mode 600), so the session banner, hooks, and scripts can read it; the plugin store is opaque to everything but the plugin. A key already present as `TYPESAFE_API_KEY` in the shell or the settings `env` block is used as is. Add one later with `claude plugin install fast-jev-compaction@fast-jev-compaction --config apiKey=<key>`. Without a key, both plugins fall back to native behavior. Fail-open: a failed `claude plugin ...` call only warns. Skipped for the light profile, when `claude` is absent, or when `CC_SETTINGS_SKIP_PLUGIN_INSTALL=1`. See [Function hooks](./hooks-reference.md#function-hooks-early-access). | Not owned (product-managed plugin store) | Never removed automatically — uninstall prints `claude plugin remove fast-jev-compaction@fast-jev-compaction && claude plugin remove compaction-trigger@cc-settings`. |
