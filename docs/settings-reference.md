...
| `ENABLE_TOOL_SEARCH` | `auto:N` | MCP tool deferral threshold. Tools deferred when descriptions exceed N% of context. cc-settings sets `auto:10` (was `auto:50` until 15.22.0): on a 1M-window model 50% never triggers, so every MCP schema rode along on every turn and made up most of a measured ~121K fixed context floor. Per-server opt-out via `alwaysLoad: true` (v2.1.121) |
