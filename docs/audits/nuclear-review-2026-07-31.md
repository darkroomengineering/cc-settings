...
3. ~~`src/scripts/codex-run.ts` — three copies of result handling, already drifted.~~ **WITHDRAWN on re-verification.** The three blocks are not parallel: `review` and `ask` share a 9-line shape, but `exec`'s success branch has ~11 lines of unique git-summary logic (`git status` / `git diff --stat` plus output sanitizing). Only the *failure* branch is genuinely identical, and its "drift" is just the subcommand name in the message — deliberate labeling, not decay. The proposed `reportResult()` would have to take exec's git-summary work as a callback, i.e. the N5 shape. A narrow failure-only helper nets about −3 lines. Not worth the churn.
