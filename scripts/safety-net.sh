...
# ---------------------------------------------------------------------------
# 3. check_ai_attribution - Block AI fingerprints in commits and PRs
# ---------------------------------------------------------------------------

check_ai_attribution() {
  local cmd="$1"

  # Check git commit messages for AI attribution
  if echo "$cmd" | grep -qE 'git\s+commit'; then
    if echo "$cmd" | grep -qiE 'Co-Authored-By.*Claude|Co-Authored-By.*Anthropic|Co-Authored-By.*noreply@anthropic'; then
      block_command "AI attribution detected in commit message — stealth mode is on" "$cmd"
    fi
    if echo "$cmd" | grep -qiE 'Generated with.*Claude|Generated by.*Claude|AI-assisted|🤖'; then
      block_command "AI fingerprint detected in commit message — stealth mode is on" "$cmd"
    fi
  fi

  # Check gh pr create for AI attribution in title or body
  if echo "$cmd" | grep -qE 'gh\s+pr\s+create'; then
    if echo "$cmd" | grep -qiE 'Co-Authored-By.*Claude|Co-Authored-By.*Anthropic|noreply@anthropic'; then
      block_command "AI attribution detected in PR — stealth mode is on" "$cmd"
    fi
    if echo "$cmd" | grep -qiE 'Generated with.*Claude|Generated by.*Claude|AI-assisted|🤖.*Claude'; then
      block_command "AI fingerprint detected in PR description — stealth mode is on" "$cmd"
    fi
  fi
...
# ---------------------------------------------------------------------------
# 4. check_git_destructive - Block dangerous git operations
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 5. check_find_xargs - Detect dangerous file operation chains
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 6. unwrap_and_analyze - Shell wrapper recursion (max depth 3)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 7. check_interpreter_oneliners - Detect dangerous interpreter invocations
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 8. split_commands - Handle multi-command strings
# ---------------------------------------------------------------------------
...
split_commands() {
  ...
  # AI attribution check must run on the full multiline string before splitting
  check_ai_attribution "$cmd"
...
main() {
  ...
  if ! echo "$COMMAND" | grep -qE '\b(rm|git|gh|find|xargs|bash|sh|zsh|python|python3|node|ruby|perl)\b'; then
    exit 0
  fi
